...
import { dbGet, dbFindBy, dbUpsert, dbDelete, configured, orderLinkSig } from "./_lib.js";
...
export default async function handler(req, res) {
  ...
  try {
    const client = await dbGet("clients", c);
    ...
    if (action === "history") {
      ...
      const mine = (await dbFindBy("orders", "clientId", c))
        .filter(o => !o.isSample && o.date && new Date(o.date) >= cutoff)
    ...
    if (action === "cancel") {
      const targets = (await dbFindBy("orders", "clientId", c)).filter(o => o.date === String(date) && o.from_client && o.status === "новая");
    ...
    if (action === "submit") {
      ...
      if (/^\d{4}-\d{2}-\d{2}$/.test(replaceDate)) {
        const olds = (await dbFindBy("orders", "clientId", c)).filter(o => o.date === replaceDate && o.from_client && o.status === "новая");
