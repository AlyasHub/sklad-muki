...
Самовывоз и погрузка (ВАЖНО):
- Клиент ЗАБИРАЕТ САМ (слова: «самовывоз», «самовывозом», «сам заберёт», «сами заберут», «заберёт сам», «забирает сам», «заберут со склада») → "pickup": true. Иначе "pickup": false.
- При самовывозе определи: ГРУЗИМ МЫ или только КОНТРОЛЬ:
  - Наш человек ГРУЗИТ (слова: «грузчик X», «загрузит X», «грузит X», «отгрузит X», «пусть загрузит X») → "pickupWatch": false, а в "worker" впиши ИМЯ этого человека как в заявке (например «Мурат»).
  - Клиент грузит САМ, мы только СМОТРИМ/контролируем (слова: «сами заберут», «сами загрузят», «сами погрузят», «бригадир наблюдает», «бригадир проследит», «только проследить», «под контролем», «проконтролировать») → "pickupWatch": true. "worker" оставь пустым "", если имя наблюдателя не названо явно.
  - Если про погрузку ничего не сказано → "pickupWatch": false, "worker": "".
- Обычная доставка (не самовывоз) → "pickup": false, "pickupWatch": false, "worker": "".
Заметка ("note"):
- Любые особые пожелания/пометки из сообщения, не относящиеся к товару/количеству/дате (например «с отлёжкой», «оставить у охраны», «позвонить перед приездом», «занести на второй этаж»). Если таких нет — пустая строка "". Слова про самовывоз/погрузку в note НЕ дублируй.
Верни JSON массив: [{"clientName":"...","brand":"...","grade":"...","bag_kg":25,"bags":40,"date":"YYYY-MM-DD","trial":false,"note":"","pickup":false,"pickupWatch":false,"worker":""}]
Только JSON.`;
