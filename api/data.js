...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 🧪 Разбор лабораторного анализа муки (только администратор). Живёт здесь, а не отдельной
    // функцией, чтобы не упереться в лимит serverless-функций Vercel (12 на Hobby).
    if (op === "parseAnalysis") {
      if (u.role !== "director") return res.status(403).json({ error: "Только для администратора" });
